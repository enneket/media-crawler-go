...
### 评论 / 媒体
- [x] 评论抓取：目前覆盖 xhs / douyin / bilibili（支持分页；douyin/xhs/bilibili 支持二级评论开关）。
...
### 评论覆盖
- [ ] 补齐评论抓取的平台覆盖：weibo / tieba / zhihu / kuaishou（Python 已实现；Go 已覆盖 xhs/douyin/bilibili）。
...
- [ ] T-201 补齐评论抓取的平台覆盖：weibo/tieba/zhihu/kuaishou（bilibili 已完成）。
