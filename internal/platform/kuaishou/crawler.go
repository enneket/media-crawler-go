...
func (c *Crawler) Run(ctx context.Context, req crawler.Request) (crawler.Result, error) {
	...
	default:
		return crawler.Result{}, fmt.Errorf("unsupported kuaishou mode: %s", req.Mode)
	}
}
...
func (c *Crawler) runSearch(ctx context.Context, req crawler.Request) (crawler.Result, error) {
	...
	if len(keywords) == 0 {
		return crawler.Result{}, fmt.Errorf("empty keywords for kuaishou search")
	}
	...
	for _, keyword := range keywords {
		...
		logger.Info("kuaishou searching keyword", "keyword", keyword)
		page := startPage
		for {
...
func (c *Crawler) runCreator(ctx context.Context, req crawler.Request) (crawler.Result, error) {
	if len(req.Inputs) == 0 {
		return crawler.Result{}, fmt.Errorf("empty inputs for kuaishou creator")
	}
	maxNotes := req.MaxNotes
	if maxNotes == 0 {
		maxNotes = config.AppConfig.CrawlerMaxNotesCount
	...
	for _, input := range req.Inputs {
		...
		creatorURL := normalizeKSCreatorURL(input)
		creatorID, _ := ParseKSCreatorID(input)
		if creatorID == "" {
			creatorID = stableID("ks_creator", input)
		}
		logger.Info("kuaishou fetch creator", "creator_id", creatorID, "url", creatorURL)

		res, err := c.client.FetchHTML(ctx, creatorURL)
		if err != nil {
			logger.Error("kuaishou creator fetch failed", "url", creatorURL, "err", err)
			continue
		}
...
func (c *Crawler) fetchAndSaveDetail(ctx context.Context, platform string, url string) error {
	...
	if config.AppConfig.EnableGetComments {
		comments := fetchCommentsPreferAPI(ctx, c.client, res.Body, noteID, ksid, config.AppConfig.CrawlerMaxComments, config.AppConfig.EnableGetSubComments)
		if len(comments) > 0 {
			...
			case "xlsx_book":
				...
				if _, err := store.AppendUniqueGlobalCommentsBook(
					...
					logger.Error("kuaishou save global comments xlsx_book failed", "note_id", noteID, "err", err)
				}
...
func normalizeKSDetailURL(input string) string {
	ksid, _, err := ParseKSID(input)
	if err == nil && ksid != "" && !(strings.HasPrefix(strings.TrimSpace(input), "http://") || strings.HasPrefix(strings.TrimSpace(input), "https://")) {
		return fmt.Sprintf("https://www.kuaishou.com/short-video/%s", ksid)
	}
	return strings.TrimSpace(input)
}

func normalizeKSCreatorURL(input string) string {
	...
	return "https://www.kuaishou.com/profile/" + url.PathEscape(s)
}
